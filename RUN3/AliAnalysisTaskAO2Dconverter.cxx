...
const TString AliAnalysisTaskAO2Dconverter::TreeName[kTrees] = { "O2collision", "DbgEventExtra", "O2trackpar", "O2trackcov", "O2trackextra", "O2calo",  "O2calotrigger", "O2muon", "O2muoncluster", "O2zdc", "O2fv0a", "O2fv0c", "O2ft0", "O2fdd", "O2v0", "O2cascade", "O2tof", "O2mcparticle", "O2mccollision", "O2mctracklabel", "O2mccalolabel", "O2mccollisionlabel", "O2bc" };
...
void AliAnalysisTaskAO2Dconverter::InitTF(ULong64_t tfId)
{
  ...
  TTree* tTracksPar = CreateTree(kTracksPar);
  if (fTreeStatus[kTracksPar]) {
    tTracksPar->Branch("fCollisionsID", &tracks.fCollisionsID, "fCollisionsID/I");
    tTracksPar->Branch("fTrackType", &tracks.fTrackType, "fTrackType/b");
    tTracksPar->Branch("fX", &tracks.fX, "fX/F");
    tTracksPar->Branch("fAlpha", &tracks.fAlpha, "fAlpha/F");
    tTracksPar->Branch("fY", &tracks.fY, "fY/F");
    tTracksPar->Branch("fZ", &tracks.fZ, "fZ/F");
    tTracksPar->Branch("fSnp", &tracks.fSnp, "fSnp/F");
    tTracksPar->Branch("fTgl", &tracks.fTgl, "fTgl/F");
    tTracksPar->Branch("fSigned1Pt", &tracks.fSigned1Pt, "fSigned1Pt/F");
    tTracksPar->SetBasketSize("*", fBasketSizeTracks);
  }
...
//      continue;
    ...
    if (fTreeStatus[kTracksPar]) ntrk_filled++;
  } // end loop on tracks
  ...
  for (Int_t itr = Ntracklets; itr--;) {
	...
	FillTree(kMcTrackLabel);
      ...
      FillTree(kTracksCov);
      FillTree(kTracksExtra);
      if (fTreeStatus[kTracksPar]) ntracklet_filled++;
    }
  } // end loop on tracklets
  eventextra.fNentries[kTracksPar] = ntrk_filled + ntracklet_filled; 
  eventextra.fNentries[kTracksCov] = eventextra.fNentries[kTracksPar];
  eventextra.fNentries[kTracksExtra] = eventextra.fNentries[kTracksPar];
